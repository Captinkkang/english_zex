<script lang="ts">
  ...
  function goPlan(type: RewardType) {
    selectedType = type;
    step = 'plan';
  }
        ...
        <div class="card-group">
          <div class="ba-card">
            ...
            <div class="card-content">
              ...
              <h3>슬렌더</h3>
              <p>다양한 매력이 있는 학생들과의 기록</p>
            </div>
            <button class="ba-start-btn" onclick={() => goPlan('A')}>
          ...
          <div class="ba-card">
            ...
            <button class="ba-start-btn" onclick={() => goPlan('B')}>
              DEPLOY <span class="arrow">▶</span>
            </button>
            <div class="card-deco"></div>
          </div>
          ...
          <div class="ba-card">
            ...
            <button class="ba-start-btn" onclick={() => goPlan('C')}>
              DEPLOY <span class="arrow">▶</span>
            </button>
            <div class="card-deco"></div>
          </div>
        ...
        <div class="plan-grid">
          {#each initialTypes as type, i}
            <div class="plan-card" class:is-syn={type}>
              ...
              <div class="reward-tag">{type ? '📷 4 PHOTOS' : '📷 1 PHOTO'}</div>
            </div>
          {/each}
        </div>

        <div class="plan-footer">
          <button class="plan-back-btn" onclick={goToMain}>CANCEL</button>
          <button class="ba-start-btn plan-confirm" onclick={startTest}>
            MISSION START <span class="arrow">▶</span>
          </button>
        </div>
      </div>
    </div>
...
<style>
  /* --- 이전 CSS 그대로 유지 --- */
  :global(:root) { --ba-blue: #00A3FF; --ba-dark: #123456; --ba-bg-light: #F3F7F9; --ba-white: #FFFFFF; --ba-gray: #7F8C8D; }
  :global(html, body) { margin: 0; padding: 0; width: 100%; height: 100%; overflow-x: hidden; background-color: var(--ba-bg-light); }
  ...
  .bg-trapezoid-overlay { position: absolute; top: 0; right: 0; width: 75%; height: 100%; background: rgba(243, 247, 249, 0.85); clip-path: polygon(15% 0%, 100% 0%, 100% 100%, 0% 100%); backdrop-filter: blur(3px); }
  .grid-overlay { position: absolute; width: 100%; height: 100%; background-image: radial-gradient(var(--ba-blue) 1px, transparent 1px); background-size: 40px 40px; opacity: 0.1; }
  .lobby-container { max-width: 1100px; margin: 0 auto; padding: 0 20px; min-height: 100vh; display: flex; flex-direction: column; }
  .ba-header { display: flex; justify-content: space-between; align-items: center; border-bottom: 3px solid var(--ba-dark); padding: 20px 15px; margin-bottom: 50px; background: rgba(255, 255, 255, 0.4); backdrop-filter: blur(10px); border-radius: 0 0 15px 15px; box-shadow: 0 4px 15px rgba(0,0,0,0.05); }
  .header-left { display: flex; align-items: center; gap: 20px; }
  .logo-symbol { width: 45px; height: 45px; background: var(--ba-blue); clip-path: polygon(25% 0%, 100% 0%, 75% 100%, 0% 100%); }
  .title-group h1 { margin: 0; font-size: 1.6rem; color: var(--ba-dark); letter-spacing: 1px; font-weight: 900; text-shadow: 1px 1px 0px white; }
  ...
  .section-title .kr { display: block; font-size: 1.3rem; font-weight: 800; color: var(--ba-dark); margin-top: -15px; padding-left: 4px; border-left: 4px solid var(--ba-blue); margin-left: 4px; text-shadow: 1px 1px 2px rgba(255,255,255,0.8); }
  .card-group { display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 30px; }
  .ba-card { background: var(--ba-white); border: 2px solid #E2E8F0; border-radius: 0 30px 0 30px; padding: 35px 30px; position: relative; overflow: hidden; transition: all 0.3s; box-shadow: 10px 10px 0px rgba(18, 52, 86, 0.05); display: flex; flex-direction: column; justify-content: space-between; min-height: 220px; }
  ...
  .ba-start-btn { background: var(--ba-dark); color: white; border: none; padding: 15px; border-radius: 12px; font-weight: 900; cursor: pointer; font-size: 1rem; display: flex; justify-content: space-between; align-items: center; transition: all 0.2s cubic-bezier(0.175, 0.885, 0.32, 1.275); box-shadow: 4px 4px 0px var(--ba-blue); }
  .ba-start-btn:hover { background: var(--ba-blue); transform: translate(-2px, -2px); box-shadow: 6px 6px 0px var(--ba-dark); }
  ...
  .ba-footer { margin-top: auto; padding: 25px 20px; background: var(--ba-dark); color: rgba(255,255,255,0.4); border-radius: 15px 15px 0 0; font-size: 0.8rem; font-weight: bold; text-align: center; }
  ...
  /* --- Plan 전용 스타일 --- */
  .plan-window { background: rgba(255,255,255,0.9); padding: 40px; border-radius: 0 40px 0 40px; border: 3px solid var(--ba-dark); box-shadow: 15px 15px 0 rgba(0, 163, 255, 0.1); margin: auto 0; }
  .plan-grid { display: grid; grid-template-columns: repeat(5, 1fr); gap: 15px; margin: 30px 0; }
  .plan-card { background: white; border: 2px solid #e2e8f0; border-radius: 15px; padding: 15px; text-align: center; }
  .plan-card.is-syn { border-color: var(--ba-blue); background: #f0faff; }
  .set-label { font-weight: 900; color: var(--ba-dark); font-size: 0.8rem; margin-bottom: 10px; }
  .type-btns { display: flex; flex-direction: column; gap: 5px; }
  .type-btns button { border: 1.5px solid #cbd5e1; background: white; padding: 8px; border-radius: 8px; font-weight: 800; cursor: pointer; font-size: 0.8rem; }
  .type-btns button.active { background: var(--ba-dark); color: white; border-color: var(--ba-dark); }
  .reward-tag { margin-top: 10px; font-size: 0.7rem; color: var(--ba-blue); font-weight: 900; }
  .plan-footer { display: flex; gap: 20px; align-items: center; margin-top: 20px; }
  .plan-back-btn { background: #e2e8f0; border: none; padding: 15px 30px; border-radius: 12px; font-weight: 900; cursor: pointer; color: #475569; }
  .plan-confirm { flex: 1; }
</style>